...
def download_diluted_average_shares(ticker):
    attempt = 1  # initialize attempt counter
    while True:
...
def download_ordinary_shares_number(ticker):
    attempt = 1  # initialize attempt counter
    while True:
        t = yq.Ticker(ticker, validate=True, status_forcelist=[404], retry=10, timeout=10, proxies={
                'http': 'http://127.0.0.1:8001 ',
        ...
        try:
            print(f"Attempt {attempt} for {ticker}")
            df = t.balance_sheet(frequency='q')
            df['asOfDate'] = pd.to_datetime(df['asOfDate'])  # convert 'asOfDate' to datetime
            df = df.rename(columns={'asOfDate': 'Date'})
            df.set_index('Date', inplace=True)  # set 'Date' as the index
            df = df[['OrdinarySharesNumber']]  # keep only the 'OrdinarySharesNumber' column
            df.columns = [ticker]  # rename the column to the ticker
            return df  # return the DataFrame
        except Exception as e:
            print(f"An error occurred: {e}. Retrying...")
...
# retrieve each company's ordinary shares number using yahooquery
print("=== retrieve each company's ordinary shares number using yahooquery ===")

with concurrent.futures.ThreadPoolExecutor() as executor:
    df_list = list(executor.map(download_ordinary_shares_number, tickers))

# Concatenate the DataFrames along the columns axis
df_shares = pd.concat(df_list, axis=1)
df_shares.insert(0, 'data_type', 'Ordinary Shares Number')
